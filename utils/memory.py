...
def save_interaction(query, response, query_info, sentiment, db_path="RAG-MODEL/user_history.db"):
    ...
    conn = sqlite3.connect(db_path)
    c = conn.cursor()
    keywords = " ".join(query_info.get("keywords", []))
...
def get_user_history(limit=10, db_path="RAG-MODEL/user_history.db"):
    ...
    try:
        ...
        history = c.fetchall()
        conn.close()
        return [{"timestamp": h[0], "query": h[1], "response": h[2], "department": h[3], "level": h[4], "semester": h[5], "sentiment": h[6], "keywords": h[7].split() if h[7] else []} for h in history]
    except sqlite3.Error as e:
        print(f"Database error: {e}")
        return []

def get_relevant_context(user_id=None, limit=5, db_path="RAG-MODEL/user_history.db"):
    """Get relevant context from long-term memory to enhance RAG queries."""
    history = get_user_history(limit=limit, db_path=db_path)
    if not history:
        return None
    # Aggregate recent context (e.g., departments, keywords)
    context = {
        "departments": [h["department"] for h in history if h["department"]],
        "levels": [h["level"] for h in history if h["level"]],
        "semesters": [h["semester"] for h in history if h["semester"]],
        "keywords": [kw for h in history for kw in h["keywords"]]
    }
    return context
